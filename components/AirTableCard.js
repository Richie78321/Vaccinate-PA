...
export default function AirTableCard({ location }) {
  ...
  const reportNotes = reportNoteList && reportNoteList[0] ? reportNoteList[0].trim() : "";
  ...
  if (availabilityStatus.value === AVAILABILITY_STATUS.UNKNOWN.value) {
    availabilityTag = (
      <span className="text-dark font-weight-bold">
        <FaQuestionCircle size="1.25em" className="mr-1" /> <span className="align-middle">No confirmation / unknown</span>
      </span>
    );
  } else if (availabilityStatus.isAvailable) {
    availabilityTag = (
      <span className="text-success font-weight-bold">
        <FaCheckCircle size="1.25em" className="mr-1" /> <span className="align-middle">Vaccines available ({availabilityStatus.display})</span>
      </span>
    );
  } else {
    availabilityTag = (
      <span className="text-danger font-weight-bold">
        <FaTimesCircle size="1.25em" className="mr-1" /> <span className="align-middle">Vaccines not available</span>
      </span>
    );
  }
          ...
          {reportNotes.length > 0 ?
            <li className="list-group-item">
              <span className="text-black">
                <BsInfoCircle size="1.25em" className="mr-1" /> <span className="align-middle"><span className="font-weight-bold">Latest info:</span> {reportNotes}</span>
              </span>
            </li> : null}
        </ul>
      </div>
